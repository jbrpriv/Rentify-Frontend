  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-grow">
        ...
        <MotionRevealSection id="features" className="scroll-mt-20 py-20 bg-[#F8FBFC]">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-0">
            <div
              className={`relative overflow-hidden rounded-[2rem] border bg-gradient-to-br p-6 md:p-10 shadow-[0_28px_90px_rgba(11,45,114,0.12)] transition-colors duration-500 ${
                roleTab === 'tenant'
            ...
            >
              ...
              <div className="relative flex flex-col gap-8 lg:flex-row">
                <div className="flex-1 space-y-5">
                  ...
                  {/* Role toggle */}
                  <div className="inline-flex rounded-full border border-[#0992C2]/20 bg-white/80 p-1 shadow-sm">
                    <div className="relative flex gap-1 overflow-hidden rounded-full">
                      ...
                      <button
                        ...
                        className={`relative z-10 flex items-center gap-1 rounded-full px-4 py-2 text-[0.75rem] font-semibold transition-colors ${
                          roleTab === 'tenant' ? 'text-neutral-900' : 'text-neutral-500 hover:text-neutral-700'
                        }`}
                      >
                        🏠 Tenant
                      </button>
                      <button
                        ...
                        className={`relative z-10 flex items-center gap-1 rounded-full px-4 py-2 text-[0.75rem] font-semibold transition-colors ${
                          roleTab === 'landlord' ? 'text-neutral-900' : 'text-neutral-500 hover:text-neutral-700'
                        }`}
                      >
                        🏢 Landlord
                      </button>
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    {!user ? (
                      <Link href="/register" className="rf-btn rf-btn-primary text-[0.8rem]">
                        Get started free <ArrowRight size={15} />
                      </Link>
                    ) : (
                      <Link href="/dashboard" className="rf-btn rf-btn-secondary text-[0.8rem]">
                        Go to dashboard <LayoutDashboard size={15} />
                      </Link>
                    )}
                    <Link href="/browse" className="rf-btn rf-btn-ghost text-[0.8rem]">
                      Preview listings
                    </Link>
                  </div>
                </div>
                ...
                <div className="relative flex-1">
                  <div key={roleTab} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    {features.map((f, i) => {
                      const Icon = f.icon;
                      return (
                        <div
                        ...
                        >
                          <div className={`mb-3 flex h-10 w-10 items-center justify-center rounded-2xl ${
                            roleTab === 'tenant' ? 'bg-[#0AC4E0]/20' : 'bg-[#0992C2]/20'
                          }`}>
                            <Icon size={18} className="text-[#0992C2]" />
                          </div>
                          <h3 className="mb-1 text-[0.9rem] font-semibold text-neutral-900">{f.title}</h3>
                          <p className="text-[0.75rem] leading-relaxed text-neutral-600">{f.desc}</p>
                        </div>
                      );
                    })}
                  </div>
