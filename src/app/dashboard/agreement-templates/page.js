                ...
                <div key={clause._id} onClick={() => toggle(clause._id)} style={{ display:'flex', alignItems:'flex-start', gap:12, padding:'12px', borderRadius:12, border:`1.5px solid ${isSelected ? '#2563EB' : '#F1F5F9'}`, background: isSelected ? '#EFF6FF' : 'white', marginBottom:8, cursor:'pointer', transition:'all 0.15s' }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
                      ...
                      <span style={{ fontSize:'0.65rem', padding:'2px 8px', borderRadius:20, background: clause.isApproved ? '#F0FDF4' : '#FFFBEB', color: clause.isApproved ? '#16A34A' : '#D97706', fontWeight:700, border: `1px solid ${clause.isApproved ? '#BBF7D0' : '#FDE68A'}` }}>
                        {clause.isApproved ? 'Approved' : 'Your Suggestion (Pending)'}
                      </span>
                    </div>
