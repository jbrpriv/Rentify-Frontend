      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
        <div className="md:hidden divide-y divide-gray-100">
          {loading ? (
            ...
            <div className="px-4 py-12 text-center text-gray-400 font-medium">No payments found for this filter.</div>
          ) : (
              ...
              return (
                <div key={payment._id} className="p-4 space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="text-sm font-bold text-gray-900">{payment.property?.title || 'N/A'}</p>
                      <p className="text-xs text-gray-500">{payment.tenant?.name || 'N/A'} → {payment.landlord?.name || 'N/A'}</p>
                    </div>
                    <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${statusBadge}`}>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="rounded-lg bg-gray-50 p-2">
                      <p className="text-gray-400">Tenant Paid</p>
                      <p className="font-black text-gray-900">{formatMoney(Number(payment.amount || 0))}</p>
                    </div>
                    <div className="rounded-lg bg-gray-50 p-2">
                      <p className="text-gray-400">Landlord Payout</p>
                      <p className="font-black text-[#0B2D72]">{formatMoney(payoutAmount)}</p>
                    </div>
                  </div>
                  ...
                  {payment.status === 'paid' ? (
                    ...
                    <button
                      ...
                      className="inline-flex w-full items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold border bg-[#E6EAF2] text-[#0B2D72] border-[#CBD5E1] hover:bg-[#DBE2ED] disabled:opacity-50"
